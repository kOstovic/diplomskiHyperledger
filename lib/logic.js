...
async function RevokeAccessFER(revoke) {  // eslint-disable-line no-unused-vars
    ...
    if(!revoke.member.transactionRevoke) {
        revoke.member.transactionRevoke = [];
    ...
    if(!revoke.universityComponent.transactionRevoke) {
        revoke.universityComponent.transactionRevoke= [];
        revoke.universityComponent.transactionRevoke.push(revoke.member.jmbag+" "+ Date().toLocaleString());
    }
    else {
        revoke.universityComponent.transactionRevoke.push(revokev.member.jmbag+" "+ Date().toLocaleString());
    }

    const memberRegistry = await getParticipantRegistry('org.szg.Member');
    await memberRegistry.update(revoke.member);
    const assetRegistry = await getAssetRegistry('org.szg.UniversityComponent');
    await assetRegistry.update(revoke.universityComponent);
    // emit an event
    const event = getFactory().newEvent('org.szg', 'MemberEvent');
    event.memberAccess = revoke;
    emit(event);
}
/**
